...
@section('content')
    <div class="container sx:px-2">
        <div class="lg:flex sx:flex sx:flex-col lg:flex-row sx:gap-3 items-center w-full 2xl:mt-20 sx:mt-24 mb-10">
            <div class="text-3xl text-[#298DFF] w-full">Категории</div>
            ...
            <div class="flex gap-x-6"><p class="lg:text-2xl sx:text-xl text-[#828282] font-normal text-nowrap">Всего
                    категорий</p><span class="lg:text-2xl sx:text-xl font-normal">{{ $total }}</span>
            </div>
                        </div>
        </div>
                @if(isset($count))
        ...
        <div class="overflow-x-auto">
            <table class="table-auto w-full text-center">
                ...
                @foreach($categories as $category)
                    <tr class="mb-4">
                        <td class="px-4 py-2 text-nowrap">{{ $category->id }}</td>
                        <td class="px-4 py-2 text-nowrap">{{ $category->title }}</td>
                        <td class="px-4 py-2 text-nowrap">
                            <a href="{{ route('admin.category.edit', $category->id) }}" class="text-white py-1 px-4 bg-[#298dff] rounded-xl">Изменить</a>
                        </td>
                        <td class="px-4 py-2 text-nowrap">
                            <form action="{{ route('admin.category.delete', $category->id) }}" method="POST" id="deleteForm{{ $category->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
