        ...
        <div class="lg:py-20 lg:px-32 sx:px-20 sx:py-20 rounded-2xl bg-white">
            <div class="max-w-[292px]">
                ...
                <form action="{{ route('developer.login.store') }}" method="post" class="flex flex-col mb-4 gap-y-5">
                    @csrf
                    <input type="text" name="em" value="{{ auth('developer')->user()->email }}" disabled id="" placeholder="email..." class="outline-none rounded-xl border border-solid border-[#c5c5c5] border-opacity-60 pl-8 py-2">
                    <input type="hidden" value="{{ auth('developer')->user()->email }}" name="email">
