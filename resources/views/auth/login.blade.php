    ...
    <div class="container sx:px-2 flex justify-center items-center">
        <div class="lg:py-20 lg:px-32 sx:px-20 sx:py-20 rounded-2xl bg-white">
            <div class="max-w-[292px]">
                ...
                <form action="{{ route('auth.login.store') }}" method="post" class="flex flex-col mb-4 gap-y-5">
                    @csrf
                    <input type="text" name="email" id="" placeholder="Email..." class="outline-none rounded-xl border border-solid border-[#c5c5c5] border-opacity-60 pl-8 py-2">
                    @error('email')
                ...
                <a href="{{ route('auth.register') }}" class="flex justify-center text-[#298DFF] mb-3">первый раз на сайте</a>
