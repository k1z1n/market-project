        ...
        <div class="lg:py-20 lg:px-32 sx:px-20 sx:py-20 rounded-2xl bg-white">
            <div class="max-w-[292px]">
                ...
                <form action="{{ route('auth.register.store') }}" method="post" class="flex flex-col mb-4 gap-y-5">
                    @csrf
                    <input type="text" name="username" id="" placeholder="Имя..." class="outline-none rounded-xl border border-solid border-[#c5c5c5] border-opacity-60 pl-8 py-2">
                    @error('username')
                ...
                <a href="{{ route('auth.login') }}" class="flex justify-center text-[#298DFF] mb-3">войти в аккант</a>
