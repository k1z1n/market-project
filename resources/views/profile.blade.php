...
<div class="container">
    ...
    <div class="text-2xl text-opacity-80 text-black mb-7">Доступно обновлений ({{ count($updatedApplications)  }})</div>
    @if(count($newApplications) > 0 or count($updatedApplications) > 0)
            ...
            <div class="border-t border-[#828282] border-opacity-40 pt-7 pb-2">
                <div class="flex justify-between items-center pb-4">
                    <div class="flex items-center gap-3.5">
                        <img src="{{ asset('storage/application-logo/' . $application->logo_image) }}" alt="" class="max-w-[76px]">
                    ...
                    <div>
                        <a href="{{ route('application.download', $application->id) }}" class="bg-[#298DFF] rounded-xl py-2.5 px-10 text-xl text-white">Скачать
                        </a>
                    </div>
                </div>
            </div>
        ...
        @foreach($newApplications as $app)
            <div class="border-t border-[#828282] border-opacity-40 pt-7">
                <div class="flex justify-between items-center pb-4">
                    <div class="flex items-center gap-3.5">
                        <img src="{{ asset('storage/application-logo/' . $app->logo_image) }}" alt="" class="max-w-[76px]">
                        <div class="flex flex-col gap-x-1">
                    ...
                    <div>
                        <a href="{{ route('application.download', $app->id) }}" class="bg-[#828282] rounded-xl py-2.5 px-10 text-xl text-white">Скачать
                        </a>
                    </div>
                </div>
            </div>
